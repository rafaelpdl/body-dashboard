...
- Measurements are stored in IndexedDB for the dashboard origin, inside whichever
  iPhone browser you open it in. Each browser keeps its own separate store.
...
Browser site data can also be cleared or evicted — WebKit drops a site's storage
after about seven consecutive days without a visit, and that applies to every iOS
browser, Chrome included. Apple Health is the authoritative source. Use the dashboard's JSON backup as an optional additional copy.
